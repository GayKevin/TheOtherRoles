                    ...
                    if (text.ToLower().StartsWith("/size ")) { // Unfortunately server holds this - need to do more trickery
                            if (AmongUsClient.Instance.AmHost && AmongUsClient.Instance.CanBan()) { // checking both just cause
                                ...
                                if (!Int32.TryParse(text.Substring(6), out LobbyLimit)) {
                                    __instance.AddChat(CachedPlayer.LocalPlayer.PlayerControl, LocalizationManager.Instance.GetString("InvalidSizeUsage"));
                                } else {
                                    ...
                                    if (LobbyLimit != GameOptionsManager.Instance.currentNormalGameOptions.MaxPlayers) {
                                        ...
                                        __instance.AddChat(CachedPlayer.LocalPlayer.PlayerControl, $"{string.Format(LocalizationManager.Instance.GetString("LobbySizeChanged"), LobbyLimit)}");
                                    } else {
                                        __instance.AddChat(CachedPlayer.LocalPlayer.PlayerControl, $"{string.Format(LocalizationManager.Instance.GetString("LobbySizeIsTheSame"), LobbyLimit)}");
                                    }
                                }
                            }
                        }
