            ...
            try {
                if (ExileController.Instance != null && ExileController.Instance.exiled != null) {
                    ...
                    if (id == StringNames.ExileTextPN || id == StringNames.ExileTextSN || id == StringNames.ExileTextPP || id == StringNames.ExileTextSP) {
                        __result = player.Data.PlayerName + $" {LocalizationManager.Instance.GetString("WasThe")} " + String.Join(" ", RoleInfo.getRoleInfoForPlayer(player, false).Select(x => x.name).ToArray());
                    }
                    // Hide number of remaining impostors on Jester win
                    if (id == StringNames.ImpostorsRemainP || id == StringNames.ImpostorsRemainS) {
                        if (Jester.jester != null && player.PlayerId == Jester.jester.PlayerId) __result = "";
                    }
                    if (Tiebreaker.isTiebreak) __result += $" ({LocalizationManager.Instance.GetString("Tiebreaker")})";
